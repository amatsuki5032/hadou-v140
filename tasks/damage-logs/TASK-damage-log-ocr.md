...
5部隊を5セッションで同時処理する。各部隊のログは独立しているため、バッチ境界の問題が発生しない。
...
│   ├── progress.json
│   └── result.csv
└── merged-result.csv     ← 全部隊統合（マージで生成）
```
...
「tasks/damage-logs/TASK.md を読んで enshou を処理して」
「tasks/damage-logs/TASK.md を読んで sousou を処理して」
「tasks/damage-logs/TASK.md を読んで ryubi を処理して」
「tasks/damage-logs/TASK.md を読んで sonken を処理して」
```
...
```
「tasks/damage-logs/ の全 */result.csv を結合して
  tasks/damage-logs/merged-result.csv に出力して。
  ヘッダー行は1行目のみ残す。」
```
...
4. **compact後の再開:** 「`tasks/damage-logs/TASK.md` を読んで kouki の続きを実行して」
...
- コミット・プッシュも不要（ローカル作業）
- **各セッションは自分のフォルダのみ操作する。他のフォルダのファイルに触れない**
- **最初からやり直したい場合は対象フォルダの progress.json と result.csv を手動削除してから再実行**
- **compact後の再開:** 「`tasks/damage-logs/TASK.md` を読んで kouki の続きを実行して」

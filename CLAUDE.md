...
│   └── treasures/
├── BACKLOG.md               ← タスクキュー
├── INBOX.md                 ← ユーザーメモ（Claude Code読み取り専用）
├── docs/                    ← ドキュメント
├── tools/                   ← ETLスクリプト（Python）
└── tasks/                   ← タスク入力ファイル（完了後削除可）
```
...
- 現在のタスク完了後、BACKLOG.md の「待機中」を確認すること
- 各タスクの着手前に影響範囲を提示し、確認を取ってから実行する
- 着手時は「作業中」に移動し、完了時は「完了」に日付付きで移動する
- 複数タスクをまとめて実行してよいが、変更ファイルが重複する場合は順番に処理する
- 1タスク完了ごとに commit & push する
- コンテキスト使用量が80%を超えたら、次のタスクに進まずユーザーの確認を待つ。80%以下なら次のタスクに続行してよい
- タスクの入力ファイル（CSV, JSON等）は `tasks/` 配下に配置する。完了後は削除してよい
- BACKLOG.md 自体の変更はコミット対象に含める
- INBOX.md はユーザーが随時メモを追記するファイル。Claude Code は読み取り専用、一切変更しないこと
- INBOX.md の各項目について、同じ趣旨の内容が既に BACKLOG.md にあればスキップする。判断に迷う場合はユーザーに確認する
...
- **git pull の直後に INBOX.md を確認する。BACKLOG.md に未登録の内容があれば転記してから本タスクに着手する**

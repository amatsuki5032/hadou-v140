...
[✅] data-research.js 生成（140件、M/専攻分類付き、no/maxValue/maxLevel/category/target付き）
...
[✅] 研究タブ: 専攻選択 + 項目入力（140件）— isMaster判定・第4枠→特化戦術リネーム修正済み、maxValue対応済み
...
[✅] ② 名宝ステータス加算 — calc-engine.js の buildAllEntries() + collectTreasureSkillEntries() で実装済み。鍛錬ランク・UR状態をprofileDataから参照。EMBEDDED_TREASURES_DATA→TREASURE_FORGE のIDマッピングは技能名セット一致で解決
[✅] ③ 兵科レベル — stat-calculator.js の TROOP_LEVEL_TABLE + getTroopLevel() で実装。レアリティ×将星から兵科Lv自動算出。攻撃・防御に兵科Lv補正（歩/騎:(Lv-1)%, 弓:(Lv-11)%）適用
...
- 名宝ステータス加算実装（buildAllEntries / collectTreasureSkillEntries）鍛錬ランク・UR状態対応
...
### v1.50+ プロファイルUI改善・兵力計算（2026-02-14〜15）

- 研究タブ: チェックボックス → 🔒/🔓 ロックアイコンに変更
- 研究タブ: 全リセットボタン追加（確認ダイアログ付き）
- 研究タブ: 一括操作ボタンの配置順序整理（低←→高、左←→右）
- 軍馬タブ: 横タブ切り替え → 3頭カード型横並びレイアウトに変更
- 軍馬タブ: 技能名に兵科サフィックス表示（勇叡→勇叡・騎 等）
- 軍馬タブ: 毛色/名馬セレクトを1つのプルダウンに統合、名馬チェックボックス廃止
- 軍馬タブ: 同一名馬の重複選択防止
- 部隊ステータスに兵力・機動・射程を追加（TROOP_LEVEL_TABLE / HP_BY_TROOP_LEVEL / UNIT_TYPE_PARAMS）
- 兵科Lv補正: 主将の兵科Lvから攻撃・防御への%ボーナス自動計算
- 兵科一致ボーナス: 副将・補佐の兵科が主将と一致で1人あたりHP+10%
- 軍馬技能（大固・兵科 / 勇叡・兵科）: %ボーナス→固定値加算に修正（データ値×100が実値）
- 軍馬技能: 兵科依存スキルの兵科不一致チェック追加（勇叡・歩が騎兵部隊に適用されない等）
- ランク設定タブの選択状態をlocalStorageに保存（hadou-rankTab）

### v1.50+ 研究データ更新（2026-02-15）

- data-research.js再生成: 91→140項目（連番ID 5000-5139、新フィールド no/maxValue/maxLevel/category/target追加）
- 研究タブ: 効果値最大の表示（maxValueベース）、入力上限のmaxValue制限、全上限設定ボタンのmaxValue対応
- 研究データETLスクリプト作成（tools/etl_research.py）
- 旧「弓平知力」→「弓兵知力①/②」に修正（data-research.js側。data-skill-db.jsは「弓平知力」残存→Excel修正待ち）、旧「城壁耐久・大」「歩兵攻撃」等を①②に分割

### v1.50+ UI改善・フィルタ拡張（2026-02-16）

- 武将リスト表示名の文字拡大（13px→15px、4文字以上11px→13px）
- 名宝リスト縦幅・アイコン拡大（min-height 36→42px、アイコン32→38px）
- レアリティタグ変更時に侍従指定をクリア（LR以外への切替も対象）
- 侍従方向選択時にLRタブ自動切替＋兵科フィルタリセット
- おススメ→好相性（武将リスト）/ 関連名宝（名宝リスト）に名称変更
- おススメ・お気に入りと勢力フィルタの排他制御
- 部隊カードに主将名・兵科を表示（「部隊N 主将名 兵科」形式）
- 参軍→武将/侍従枠へのD&D時に参軍がクリアされないバグ修正
- 遠射・敏活フィルタタグ追加（SKILL_DBから射程/戦法速度エフェクト自動検出、武装タイプ除外）
- 袁紹タグの範囲拡大（±10→±13、皇甫嵩 affinity:88 を含む）
- 武将/名宝リストのタグ選択欄の位置合わせ（パネルヘッダー間隔圧縮）
- 簡易モード時の陣形・参軍 表示切替ボタン追加（localStorage保持）

### v1.50+ 簡易モードレイアウト改善（2026-02-17）

- 簡易モードで陣形・参軍非表示時の右側空白を解消（template-slots flex:1化、slot-row最終列 minmax化）
- 参軍セクションを flex:0 0 auto に変更（陣形非表示+参軍表示時のレイアウト崩れ修正）
- SSR袁術の不在確認（data-generals.jsに袁術なし→Excel修正待ち）
- SSR羊祜の重複確認（id:4052がステータス全0の空データ→Excel修正待ち）

### v1.51 フィルタ・UX強化（2026-02-17）

- 簡易モード陣形グリッドのアイコンはみ出し修正（formation-gridクラス追加、item-icon 36px制限）
- 武将・名宝タグに即壊滅回避・与ダメージ・被ダメージフィルタ追加（SKILL_DB自動検出、武将/名宝で独立state）
- 武将スキル系タグON時にレアリティタブを全展開（遠射・敏活・即壊滅回避・与ダメージ・被ダメージ）
- D&D・ダブルクリック配置時に対象部隊を自動で注目マークに設定（handlers-dnd.jsにsetRecommendTargetFormation連携）
- 名宝「関連名宝」タグを関連名宝（related一致）+練達名宝（スキル練達マッチ）に分割、お気に入りと排他制御、ON時にカテゴリ全展開+兵科リセット
- 戻すボタンを10回まで対応（undoHistoryを配列化、ボタンに残回数表示）
- 武将敏活タグに武装スキル除外条件追加（type1!=='武装'、遠射・即壊滅回避・与ダメ・被ダメと統一）
- 編制パネルからダブルクリック削除時も注目部隊に設定（武将・侍従・参軍・名宝の4ハンドラにsetRecommendTargetFormation追加）
- 部隊の並び替え機能追加（▲▼ボタンでデータスワップ、折りたたみ・注目部隊追従）
- SKILL_DBスキャンを6個のuseMemoから1個のskillNameSetsに統合（1回スキャンで6 Set同時構築）
- 削除ハンドラ4個をhandleRemoveSlot共通関数に統合（外部API維持）

### v1.52 推奨陣形ファインダー（2026-02-18）

- 推奨陣形ファインダー（formation-finder.html）を新規作成
- 各スロットの侍従方向を指定して全19陣形の侍従配置可能数を比較するツール
- resolveAttendantConflicts（utils.js）を再利用、ダミーLRオブジェクトで評価
- 配置可能数降順ソート、3×3ミニグリッド描画、スロット別配置可否バッジ
...
### 7.4 推奨陣形ファインダー（formation-finder.html）

リポジトリルート（index.htmlと同階層）に配置。

- 各スロット（主将/副将1/副将2/補佐1/補佐2）の侍従方向をドロップダウンで指定
- 全19陣形に対して `resolveAttendantConflicts`（utils.js）で侍従配置をシミュレーション
- 配置可能侍従数の多い順にカード表示（3×3ミニグリッド付き）
- 依存: config.js（FORMATIONS_TYPES）、utils.js（侍従配置ユーティリティ）
- URL: `https://amatsuki5032.github.io/hadou-v140/formation-finder.html`

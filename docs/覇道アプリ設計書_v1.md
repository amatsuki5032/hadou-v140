...
SKILL_EFFECTS_DATA（499技能）── 手動抜粋 → 廃止予定（SKILL_DBに完全包含）
COMBAT_PARAMETERS （171技能）── 手動抜粋 → 廃止予定（SKILL_DBに完全包含）
...
[ ] 旧data-skill-effects.js / data-combat-parameters.js 廃止（参照なし確認後）
```
...
[ ] stat-calculator.js統合（研究の効果→部隊ステータス反映）— DB上にレベル別効果値なし、ユーザー入力式で対応予定
```
...
```
[ ] ① 参軍ステータス加算 — 優先度:高。UI有・calc-engineにロジック無。参軍レベル(0-10)は☆ランクとは別概念
[ ] ② 名宝ステータス加算 — 優先度:高。前提のdata-treasure-forge.js生成済み。計算ロジック未実装
[ ] ③ 兵科レベル — 優先度:中。データ無（ユーザー入力式）
[ ] ④ SSR/SR/R基礎ステータス — 優先度:中。武将DBに値0。Excel入力 or ユーザー入力
[ ] ⑤ 研究レベル別効果値 — 優先度:中。DB無（ユーザー入力式）。プロファイル研究タブと連動
[ ] ⑥ 陣形固有効果 — 優先度:低。59技能・3カテゴリ（能力変化/特殊効果/条件効果）に分類済み
...
| ファイル | 行数 | 責務 |
|---------|------|------|
| app.js | ~2,500 | メインアプリ（state管理・レンダリング） |
| components-panels.js | ~1,500 | GeneralsPanel, TreasuresPanel |
| components-modals.js | ~1,100 | 6つのモーダルコンポーネント |
| components-formations.js | ~400 | 編制パネル（ステータス表示・パラメータ） |
| components-rank.js | ~400 | 武将育成プロファイルUI（Lv/Grade/☆） |
...
| config.js | ~50 | 設定定数 |
| utils.js | ~350 | 共通ユーティリティ |
| updateHistory.js | ~100 | 更新履歴表示 |

旧app.js: 7,081行 → 分割後: ~2,200行（**-69%**）

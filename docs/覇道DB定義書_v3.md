...
| 12 | UR | str | UR化の有無（〇） |
| 13 | 古豪 | str | 古豪名宝フラグ（〇） |
| 14 | 成長例外 | str | 空=標準成長、〇=例外（個別倍率） |
| 15 | 統率 | int | Lv1☆0の統率値（空=0） |
| 16 | 武力 | int | Lv1☆0の武力値（空=0） |
| 17 | 知力 | int | Lv1☆0の知力値（空=0） |
| 18 | 政治 | int | Lv1☆0の政治値（空=0） |
| 19 | 魅力 | int | Lv1☆0の魅力値（空=0） |

**名宝ステータス計算式（標準名宝共通）:**

```
合計ステータス = 初期合計 × (1 + 5×(Lv-1)/49 + 1.8×ランク)
Lv50時 = 初期合計 × (6 + 1.8 × ランク)
```

- SSR標準: Lv1合計 = 20
- UR: Lv1合計 = 24（SSRの1.2倍、ただし配分比率が変わる場合がある）
- 成長例外の名宝は共通式が適用できない（個別対応が必要）
...
| 11 | ~~名宝ステータス加算~~ | ~~高~~ | ✅ 完了。calc-engine.js の buildAllEntries() + collectTreasureSkillEntries() で実装。鍛錬ランク・UR状態をprofileDataから参照 |
